                    ...
                    foreach( $location_menus as $location_menu_slug    =>  $location_menu_data ) 
                        {
                            ...
                            $menu_title = apply_filters('apto/admin/menu_title', 'Re-Order', $location_menu_data['post_type']);
            ...
            function nottice_similar_sorts($interface)
                {
                    ...
                    $args = array(
                                    ...
                                    'order'                 => 'ASC',
                                    'posts_per_page'        =>  -1,
                                    'post__not_in'          =>  array($interface->sortID),
                                    'ignore_custom_sort'    =>  TRUE
                                    );
                    $custom_query = new WP_Query($args);
                    if($custom_query->have_posts())
                            ...
                            while($custom_query->have_posts())
                                {
                                    ...
                                    if((count($interface->sort_settings['_rules']['post_type']) != count($sort_settings['_rules']['post_type']))
                                        ||
                                        count(array_diff($interface->sort_settings['_rules']['post_type'], $sort_settings['_rules']['post_type'])) > 0 
                                        )
                                        {
                                    ...
                                    if(
                                        $interface->sort_settings['_rules']['taxonomy_relation']    !=  $sort_settings['_rules']['taxonomy_relation']
                                        ||
                                        (count($interface->sort_settings['_rules']['taxonomy']) != count($sort_settings['_rules']['taxonomy']))
                                        ||
                                        $this->taxonomy_settings_diff_exist($interface->sort_settings['_rules']['taxonomy'], $sort_settings['_rules']['taxonomy']) === TRUE
                                        )
                                        {
                                    ...
                                    if(
                                        $interface->sort_settings['_rules']['meta_relation']    !=  $sort_settings['_rules']['meta_relation']
                                        ||
                                        (count($interface->sort_settings['_rules']['meta']) != count($sort_settings['_rules']['meta']))
                                        ||
                                        $this->meta_settings_diff_exist($interface->sort_settings['_rules']['meta'], $sort_settings['_rules']['meta']) === TRUE
                                        )
                                        {
                                    ...
                                    if((count($interface->sort_settings['_rules']['author']) != count($sort_settings['_rules']['author']))
                                        ||
                                        count(array_diff($interface->sort_settings['_rules']['author'], $sort_settings['_rules']['author'])) > 0 
                                        )
                                        {
                                    ...
                                    if(
                                        (count($interface->sort_settings['_conditionals']) != count($sort_settings['_conditionals']))
                                        ||
                                        $this->conditional_settings_diff_exist($interface->sort_settings['_conditionals'], $sort_settings['_conditionals']) === TRUE
                                        )
                                        {
