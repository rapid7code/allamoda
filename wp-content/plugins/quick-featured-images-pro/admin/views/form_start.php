...
foreach ( $this->valid_actions_without_image as $name => $label ) {
	if ( 'assign_first_img' == $name ) {
		$desc = '.<br />' . __( 'The plugin will try to find the first image embedded in the content, in a WordPress standard gallery or attached to a post.', $this->plugin_slug );
		if ( $this->is_ngg2 ) {
			$desc .= ' ' . __( 'It will also search in NextGen galleries.', $this->plugin_slug );
		}
	} else {
		$desc = '';
	}
?>
		<p>
			<input type="radio" id="<?php echo $name; ?>" name="action" value="<?php echo $name; ?>" <?php checked( 'assign' == $name ); ?> />
			<label for="<?php echo $name; ?>"><strong><?php echo $label; ?>.</strong></label><?php echo $desc; ?>
		</p>
<?php
}
?>
