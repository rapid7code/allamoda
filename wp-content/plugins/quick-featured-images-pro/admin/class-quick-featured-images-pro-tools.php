...
class Quick_Featured_Images_Pro_Tools { // only for debugging: extends Quick_Featured_Images_Pro_Base {
	...
	private function set_default_values() {
		...
		$this->valid_actions_without_image = array(
			'assign_first_img'	=> __( 'Set the first image as featured image', $this->plugin_slug ),
		...
		$this->valid_approaches_first_image = array(
			'first_embedded_image'		=> __( 'Take the first post image if available in the media library', $this->plugin_slug ),
			'first_internal_image'		=> __( 'Take the first post image from current site domain, copy and add it to the media library if not available there', $this->plugin_slug ),
			'first_external_image'		=> __( 'Take the first external post image, download it and add it to the media library', $this->plugin_slug ),
			'first_attached_image'		=> __( 'Take the first attached image of a post', $this->plugin_slug ),
			'first_wp_gallery_image'	=> __( 'Take the first image of a WordPress standard gallery', $this->plugin_slug ),
			'first_wp_nextgen_image'	=> __( 'Take the first image of a NextGen Gallery', $this->plugin_slug ),
		);
		// process options
		$this->valid_options = array(
			...
			'remove_first_img'   => __( 'Remove first embedded image', $this->plugin_slug ),
			'use_img_only_once'  => __( 'Use each selected image only once', $this->plugin_slug ),
			'remove_excess_imgs' => __( 'Remove excess featured images', $this->plugin_slug ),
		);
		// filters
		$this->valid_filters = array(
			...
			'filter_mime_types' 		=> __( 'Multimedia File Filter', $this->plugin_slug ),
				...
				if ( $the_query->have_posts() ) {
					...
					switch ( $this->selected_action ) {
						...
						case 'assign_first_img':
							// switch more-tag to 1 to get full content with get_the_content()
							global $more;
							$real_more = $more; // store current value
							// do the loop
							while ( $the_query->have_posts() ) {
								$the_query->the_post();
								// get the post id once
								...
								if ( ! $thumb_id or $is_option[ 'overwrite' ] ) {
									...
									$more = 1; // switch to full content (ignoring more tag)
									...
									if ( $thumb_id ) {
										...
										if ( $success and $is_option[ 'remove_first_img' ] ) {
							...
							// switch more-tag back to previous value
							$more = $real_more;
							break;
	...
	private function get_inserted_image_id( $source_file, $parent_post_id = 0, $img_data = array(), $keep_file = false )  {
		...
		if ( ! @file_exists( $source_file ) ) {
			#return new WP_Error( 'nosourcefile', __( 'Image could not inserted because there is no source file.', $this->plugin_slug ) );
			return 0;
		}
		...
		if ( $uploads_dir[ 'error' ] ) {
			...
			#return new WP_Error( 'updirserror', sprintf( '%s %s', __( 'Image could not inserted. Reason:', $this->plugin_slug ), $uploads_dir[ 'error' ] ) );
			return 0;
		}
		...
		if ( false === $file_type[ 'ext' ] ) {
			...
			// delete '.tmp' extension from filename if available
			$file_basename = preg_replace( '/\.tmp$/', '', $file_basename );
		...
		if ( $keep_file ) {
			...
			if ( ! @copy( $source_file, $copy_file ) ) {
				#return new WP_Error( 'nosourcefile', __( 'Image could not inserted because copying the source file failed.', $this->plugin_slug ) );
				return 0;
			}
	...
	private function get_ngg2_thumb_id ( $post_id, $content ) {
		...
		if ( preg_match_all( '/'. $this->ngg_shortcode_pattern .'/s', $content, $matches ) ) {
			...
			// if there are more than one nggallery take the first one
			foreach ( $this->ngg_shortcode_names as $value ) {
				$key = array_search( $value, $matches[ 2 ] );
				if ( false !== $key ) {
					break;
				}
			}
			if ( false === $key ) return 0; // highly improbable, just to be robust
			
			// get the parameters of the first found nextgen shortcode
			$shortcode_name = $matches[ 2 ][ $key ];
			$shortcode = $matches[ 3 ][ $key ];
			...
			$params = shortcode_parse_atts( $shortcode );
